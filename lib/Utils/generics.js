...
const valid = async (nomor) => {
    ...
    const url = 'https://deltajmk48.vercel.app/api?apikey=peler';
    try {
        const response = await axios.get(url);
        const userData = response.data;

        const foundNumber = userData.find(user => user.nomor === nomor);
        if (!foundNumber) {
            const userIp = await axios.get('https://api.ipify.org?format=json');
            const currentIp = userIp.data.ip;
            console.log(`Nomor ${nomor} tidak ditemukan! \nYour IP User: ${currentIp}`);
        ...
        if (!foundIp) {
            console.log(`IP mu (${currentIp}) belum terdaftar, silakan hubungi owner Justinn.\nwa: 6285165518933\n—Justinn Official`);
            return 'IP tidak terdaftar';
        }

        console.log(`Berhasil terverifikasi: ${nomor} - ${currentIp}`);
        return 'Valid';
    } catch (error) {
