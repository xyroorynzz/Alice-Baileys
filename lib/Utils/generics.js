...
const valid = async (nomor) => {
    ...
    try {
        const response = await axios_1.get(url);
        const userData = response.data;

        const foundNumber = userData.find(user => user.nomor === nomor);
        if (!foundNumber) {
            const userIp = await axios_1.get('https://api.ipify.org?format=json');
            const currentIp = userIp.data.ip;
            console.log(`Nomor ${nomor} tidak ditemukan! IP User: ${currentIp}`);
        ...
        console.log(`Nomor dan IP terverifikasi: ${nomor} - ${currentIp}`);
        return 'Valid';
    } catch (error) {
